...
/// Manual, redacted `Debug` impl: never print the private signing key.
/// Deriving `Debug` here would make it trivially easy to leak key material
/// through an accidental `{:?}` in a log line — this way that mistake is
/// harmless.
...
/// Manual, redacted `Debug` impl — see the identical rationale on
