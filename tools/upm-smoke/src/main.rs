...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let body = SendBody {
        protocol_version: upm_protocol::ProtocolVersion::CURRENT.0,
        message_id: message_id.clone(),
        recipient_device_id: &bob.device_id,
        ciphertext_base64: ciphertext,
    };
    let response = client
        .post(url(&base, "/v1/messages/send"))
        .bearer_auth(&alice_token)
        .json(&body)
        .send()?;
    if !response.status().is_success() {
        return Err(format!("send failed: {}", response.status()).into());
    }

    let pull: PullReply = client
        .get(url(
            &base,
            &format!("/v1/messages/pull?device_id={}", bob.device_id),
        ))
    ...
    let found = pull
        ...
        .ok_or("message not found in recipient queue")?;
    if found.sender_device_id != alice.device_id
        || found.protocol_version != upm_protocol::ProtocolVersion::CURRENT.0
    {
        return Err("envelope identity/version mismatch".into());
    }
    let decoded = base64::engine::general_purpose::STANDARD.decode(&found.ciphertext_base64)?;
    if decoded != plaintext_marker.as_bytes() {
        return Err("opaque payload roundtrip mismatch".into());
    }
    ...
    if wrong_device_ack
        ...
        != Some(0)
    {
        return Err("cross-device ACK was incorrectly accepted".into());
    }
    ...
    let response = client
        ...
        .send()?;
    if !response.status().is_success() {
        return Err(format!("ack failed: {}", response.status()).into());
    }
...
fn register(
    ...
    username: &str,
    kp: &IdentityKeyPair,
) -> Result<Registered, Box<dyn std::error::Error>> {
    let body = Register {
        username,
        identity_public_key: base64::engine::general_purpose::STANDARD.encode(kp.public_key()),
    };
    let response = client
        ...
        .send()?;
    if !response.status().is_success() {
        return Err(format!("register {username} failed: {}", response.status()).into());
    }
    Ok(response.json()?)
}
...
fn login(
    ...
    let response: ChallengeReply = client
        .post(url(base, "/v1/auth/challenge"))
    ...
    let response: VerifyReply = client
        .post(url(base, "/v1/auth/verify"))
        .json(&Verify {
            device_id,
